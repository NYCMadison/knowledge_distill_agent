...
- `assign_and_migrate.py` exits immediately with error if `KB_VENTURE` is not set — always provide it via env var when testing that script.
...
- Collection fetches from live URLs; some may time out or return 403 depending on network conditions.
- History deduplication is stored in `distill_input_history.json` at the project root. Re-running collection won't duplicate already-fetched URLs.
